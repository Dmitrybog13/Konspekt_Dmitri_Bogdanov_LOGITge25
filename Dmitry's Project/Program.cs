    ...
    internal class Program //<-- programmi klass, mis on ka omakorda konteiner tüüp, Kus sees on kogu programmi kood 
    {
                                        ...
                                        //käivituseks määratud
        {
            ...
            */
			...
			/* 1 - MASSIIV */
			// []		 - > massiiv on komposiitandmetüüb, mille sees saab olla mitmeid sama tüüpi lihtandmeid. Massiivi tähistatakse kantsulgudega.
			...
			//			komposiitandmetüüpidest. Kui on moodustatud massiiv mis koosneb teistest massividest on tegu mitmedimensioonilise massiiviga.
			//			Näiteks on 2D massiiv selline massiiv kus on sees omakorda veel kaks lithandmetüübist koosnevat massiivi, näiteks float.
			// Esimene tekitusviis:
			int[] uusMassiiv = new int[3];  //andmetüüü int väljendab, et tegu on täisarvutüüpi andmega, mis on massiiv, ja omab endas täisarve
											//muutuja nimi on "uusMassiiv" ning võrdusmärgi taga omistatakse sinna massiivi kolme elemendiga,
											//kus mitte elemend hetkel veel väärtust ei oma. Kui mitu elementi väljendatakse katsulgude vahel,
											//peale andmetüübi ja peale kaitstud sõna "new". New ütleb, et tegu on uue väärtusega selle muutuja sees.
			...
			int[] uusMassiiv2 = [1, 2, 3];	//Teine massiivi tekitusviis kus massiivi pikkuse sätestamise asemel antakse muutujasse kohe kindlate
											//väärtustega elemendid kaasa. Massiivi pikkus sätestama ei pea, kuna see tuletatakse elementide
			...
			// Tsüklid on programeerimise viis, kuidas automatiseerida korduv tegevus ära, see aitab hoida andmemahtu kokku, programeerija sama
			...
			/* -= T S Ü K L I D     DO-WHITE =-                     */

			// Do-white tsükkel erineb kõigist teistest tsüklitest sellega, et tingimusekontroll toimub pärast esimest tegevusringi. Kõikides
			...
			//                                        //ehk tsüklimuutuja
			//do                                      //"do" on kaistud sõna, mis alustab do while tsüklit, Sellele järgneb tsülöis tehtav koodiplokk
			//{
			//    Console.WriteLine(tsükliMuutuja1);  //Koodiploki sees olev tegevus, mis antud juhul kuvatakse tsüklimuutuja arv välja
			...
			//    //OTSESELT, ET SÜSTEEM SAAKS EDASI TEISTE TEGEVUSE JUURDE MINNA, ILMA SELLETA SEE EI OLE VÕIMALIK
			...
			// "Console" on moodul mille abil saab kasutada käsurida
			// "." abil saab adresseerida moodulist console erinevaid meetodeid
			// "WriteLine on meetod konsooli jaoks, mis kuvab käsuritta infot
			// "()" sulud on meetodi töö jaoks vajalik, kuna selle vahel on parameeter meetodi jaoks
			// "Hello, World!" on sõne, mida meetod writeline käsuritta kuvab
			// lause lõpetab lauselõpumärk ";"

			/* -= T S Ü K L I D     WHILE =-                        */
			// While tsükkel on kõige tüüpilisemat laadi tsükkel, tal on tingimuskontroll esimese ringi algauses, kuid tsükkel ei oma sisseehitatud
			// tsüklimuutujat, selle peab sätestama programeerija
			string tsükliMuutuja2 = "10";               //Mingisugune muutuja, mis omab tsükli töö tingimuse abil kontrollimiseks vajalikku väärtus
                                                        //ehk tsüklimuutuja
            while (tsükliMuutuja2 != "0")                             //"while" on kaitstud sõna, mis alustab while tsüklit, sellele järgneb sulgude vahel "()" olev
                                                                      //tingimuskontroll, kus kontrollitakse tsüklimuutuja hetkeseisu, ning kui avaldis tagastab "true"
            ...
            //For tsükkel, võrreldes dowhile või while tsükliga omab endas tsükli töö edasviimiseks tsükli enda sätestuse sees, peale tingimuskontrolli
            ...
            for (int k = 0; k < arvuMassiiv.Length ; k--)       //Kaitstud sõna "for" alustab for tsüklit, pärast mida on sulud, mille vahel on kõik tsükli töö
                                                                ...
                                                                //Teine parameeter on tsükli töö kontrollimiseks vajalik tingimuskontroll. Samamodi nahu while voi
                                                                ...
                                                                //lauselõpumärgiga välja arvatud viimane, viimase lõpetav sulg.
            ...
            {
                Console.Write("sisesta " + k". arv");//Tegevus koodiploki sees, kus küsitatakse kasutades tsüklimuutujat, kasutajalt
                                                     //järgmist/mingindat arvu.
                arvuMassiiv[k] = int.Parse(Console.ReadLine());//Kasutades tsüklimuutujat, täidetakse masiiv.
            }
